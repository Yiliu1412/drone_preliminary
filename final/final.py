...
def get_image(client: airsim.MultirotorClient):
    ...
    scene_data = np.frombuffer(scene_image.image_data_uint8, dtype=np.uint8).reshape(scene_image.height,
                                                                                     scene_image.width,
                                                                                     3)
    depth_data = np.asarray(depth_image.image_data_float, dtype=np.float32).reshape(depth_image.height,
                                                                                    depth_image.width)
...
if __name__ == '__main__':
    ...
    for i in range(len(circle_point)):
        ...
        pnp_coordinate = find_circle_position_pnp(scene_image, depth_image)
        if pnp_coordinate is None:
            continue
        else:
            cx, cy, cz, r = pnp_coordinate
        ...
        client.moveToPositionAsync(wx, wy, wz + 0.35, 5.3).join()
    ...
    client.moveToPositionAsync(-62.9, -102.3, -2.4, 4.75).join()
    client.hoverAsync().join()
    time.sleep(2)
